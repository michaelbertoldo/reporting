...
async function handle(rpc: RpcRequest, ctx: AgentToolContext, scopes: string[]): Promise<any | null> {
  switch (rpc.method) {
    case 'initialize':
    ...
    case 'tools/call': {
      ...
      if (tool.scope === 'write' && !scopes.includes('write')) {
        return ok(rpc.id, { content: [{ type: 'text', text: `This API key lacks write scope for ${name}` }], isError: true })
      }
      try {
...
export async function POST(req: NextRequest) {
  ...
  if (Array.isArray(body)) {
    const responses = (await Promise.all(body.map(r => handle(r, ctx, auth.scopes)))).filter(Boolean)
    return responses.length ? NextResponse.json(responses) : new NextResponse(null, { status: 202 })
  }

  const response = await handle(body, ctx, auth.scopes)
