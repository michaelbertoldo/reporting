...
export async function GET() {
  ...
  const add = (d: any) => { if (d && !byId.has(d.id)) byId.set(d.id, { id: d.id, title: d.title, file_name: d.file_name, mime_type: d.mime_type, size_bytes: d.size_bytes, uploaded_at: d.uploaded_at }) }

  // Fund-wide docs for the LP's enabled funds.
  const { data: fundDocs } = await (admin as any)
    .from('lp_documents')
    .select('id, title, file_name, mime_type, size_bytes, uploaded_at')
    .eq('scope', 'fund')
    .in('fund_id', enabledFundIds)
  for (const d of (fundDocs ?? [])) add(d)

  // Investor-scoped docs shared with the LP's investors.
  const { data: shares } = await (admin as any)
    .from('lp_document_shares')
    .select('lp_documents(id, title, file_name, mime_type, size_bytes, uploaded_at, scope, fund_id)')
    .in('lp_investor_id', investorIds)
  for (const s of (shares ?? [])) {
    const d = (s as any).lp_documents
    if (d && d.scope === 'investor' && enabledFunds.has(d.fund_id)) add(d)
  }
